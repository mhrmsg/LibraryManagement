...
public class HomeDaoImpl implements HomeDao {
    ...
    public List<BookBean> searchBooksByConditionCateogry(String c_name) throws Exception {
        String sql = "select * FROM book WHERE c_id = (select c_id FROM bclass WHERE  c_name like ? limit 1 )";
    ...
    public BookBean searchByBid(String bid) throws Exception {
        ...
        BookBean query = queryRunner.query(sql, new BeanHandler<BookBean>(BookBean.class), bid);
        return query;
    }

    @Override
    public String searchByCId(String cid) throws Exception {
        String sql = "select c_name from bclass where c_id = ?";
    ...
    public List<BclassBean> searchAllClass() throws Exception {
        String sql = "select c_id,c_name from bclass";
        List<BclassBean> query = queryRunner.query(sql, new BeanListHandler<BclassBean>(BclassBean.class));
        return query;
    }
    ...
    public int addNewBook(BookBean bookBean, String className) throws Exception {
        ...
        queryRunner.update(sql, null, className);
        sql = "select c_id from bclass where c_name = ?";
        Object query = queryRunner.query(sql, new ScalarHandler(), className);
        int c_id = Integer.parseInt(query.toString());
        sql = "insert  into book(b_id,b_name,b_author,b_publish,b_info,b_language,b_price,b_pdate,b_num,c_id,u_comment,u_tag,u_id)" +
                "values(?,?,?,?,?,?,?,?,?,?,?,?,?)";
        int update = queryRunner.update(sql, null, bookBean.getB_name(), bookBean.getB_author(), bookBean.getB_publish(), bookBean.getB_info(), "中文", bookBean.getB_price(), bookBean.getB_pdate(), bookBean.getB_num(), c_id, null, bookBean.getU_tag(), null);
        return update;
    }

    @Override
    public int addNewBook(BookBean bookBean) throws Exception {
        String sql = "insert into book(b_id,b_name,b_author,b_publish,b_info,b_language,b_price,b_pdate,b_num,c_id,u_comment,u_tag,u_id)" +
                "values(?,?,?,?,?,?,?,?,?,?,?,?,?)";
        Object[] params = {null, bookBean.getB_name(), bookBean.getB_author(), bookBean.getB_publish(), bookBean.getB_info(), "中文", bookBean.getB_price(), bookBean.getB_pdate(), bookBean.getB_num(), bookBean.getC_id(), null, bookBean.getU_tag(), null};
    ...
    public int searchByCName(String cname) throws Exception {
        String sql = "select c_id from bclass where c_name = ?";
        Object query = queryRunner.query(sql, new ScalarHandler(), cname);
        if (null == query) {
            return 0;
        }
    ...
    public List<BookBean> SearchDelBook(BookBean bookBean) throws Exception {
        String sql = " select * from book where b_id = ? or  b_name like ? or c_id = ? or b_author like ? or b_publish like ? ";
        String likeName = null, likeautor = null, likepublish = null;
        if (null != bookBean.getB_name() && "" != bookBean.getB_name().trim())
            likeName = "%" + bookBean.getB_name().trim() + "%";
        if (null != bookBean.getB_author() && "" != bookBean.getB_author().trim())
            likeautor = "%" + bookBean.getB_author().trim() + "%";
        ...
        Object[] params = {bookBean.getB_id(),  likeName, integer, likeautor, likepublish};
        List<BookBean> query = queryRunner.query(sql, new BeanListHandler<BookBean>(BookBean.class), params);
        return query;
    }
    ...
    public int delBook(String b_id) throws Exception {
        ...
        int update = queryRunner.update(sql, b_id);
        return update;
    }
    ...
    public UserBean searchUNameById(String u_id) throws Exception {
        ...
        return queryRunner.query(sql,new BeanHandler<UserBean>(UserBean.class),u_id);
    }

    @Override
    public int addComment(String b_id, String comment) throws Exception {
        String sql = "update book set u_comment = ? where b_id = ?";
        return queryRunner.update(sql,comment,b_id);
    }

    @Override
    public List<UserBean> searchBLend(String b_id) throws Exception {
        String sql = "select * from user where (s_id  in (SELECT s_id FROM schoolcard where b_id = ? order by b_lendDate desc))";
        return queryRunner.query(sql,new BeanListHandler<UserBean>(UserBean.class),b_id);
    }
...
//            String u_id = "";
//            for (int j = 0; j < 10; j++) {
//                 u_id +=  j + ":" + r.nextInt(300) +",";
//            }
//            String u_comment  = r.nextInt(300) + ":" + "随机评论"+RandomDataUtils.getRandomBName()+",";
//            queryRunner.update(sql, null, RandomDataUtils.getRandomBName(), RandomDataUtils.getRandomJianName(), RandomDataUtils.getRandomPublish(), RandomDataUtils.getRandomMsg(), "汉语", r.nextInt(200), RandomDataUtils.getRandomBirth(), r.nextInt(100), r.nextInt(12), u_comment, RandomDataUtils.getRandomTag(), u_id);
//        }
    }
